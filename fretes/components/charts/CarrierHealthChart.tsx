...
import type { CarrierHealthScore } from "@/lib/metrics/types";
...
export function CarrierHealthChart({ data }: { data: CarrierHealthScore[] }) {
  if (data.length === 0) {
    ...
    <ResponsiveContainer width="100%" height={Math.max(180, data.length * 36)}>
      <BarChart data={data} layout="vertical" margin={{ top: 4, right: 24, bottom: 4, left: 8 }}>
        ...
        <Bar dataKey="score" radius={[0, 4, 4, 0]} maxBarSize={20}>
          {data.map((entry) => (
            <Cell key={entry.carrier} fill={colorFor(entry.score)} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
