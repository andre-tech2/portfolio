    ...
    <div className="min-h-screen w-full flex items-center justify-center relative z-10 px-4">
      <div className="max-w-sm w-full">
        <div className="text-center mb-8">
          <Logo className="w-14 h-14 mx-auto mb-4 rounded-2xl shadow-glow" />
          <h1 className="text-2xl font-bold text-text-primary">
            <span className="brand-text">NovaTech</span> Inventário
          </h1>
          <p className="text-text-secondary mt-1 text-sm">Entre com sua conta para continuar</p>
        </div>

        <form onSubmit={handleSubmit} className="glass rounded-2xl shadow-card p-5 space-y-3">
          <div>
          ...
          <button
            type="submit"
            disabled={busy}
            className="w-full px-4 py-2.5 text-sm font-semibold bg-gradient-to-r from-accent-cyan via-accent to-accent-violet text-white rounded-md disabled:opacity-60 shadow-glow"
          >
            {busy ? 'Entrando…' : 'Entrar'}
          </button>
        </form>

        <div className="glass-raised rounded-2xl p-4 mt-4 text-xs text-text-secondary space-y-2">
          ...
          <button
            ...
            className="w-full px-3 py-2 text-xs font-semibold glass rounded-md hover:bg-surface-raised/60 transition-colors disabled:opacity-60"
          >
